...
class DSMongoModel{
     ...
     * )
     */
    public function __construct($config=array('host'=>'','port'=>'','cmd'=>'$')){
        $server = sprintf("mongodb://%s:%s", $config['host']?:DsConfig::dsConst()['MONGO_HOST'], $config['port']?:DsConfig::dsConst()['MONGO_PORT']);
    ...
    {
        return array_search('DataSlicing',array_column($this->_mongo->allDB()['databases'],'name'));
    }
     ...
     * @return mixed
     */
    public function takeOne($table = '',$val = '')
    {
        return $this->_mongo->selectDB("DataSlicing")->execute('function () { return DataSlicing.'.$table.'.find({task_id:'.$val.'}); }');
    }
